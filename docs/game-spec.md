...
1. **인트로 / Start To Battle 화면**
   - 첫 진입 시, 게임 설명과 `START TO BATTLE` 버튼이 있는 인트로 카드가 보인다.
   - 버튼을 눌렀을 때만 실제 배틀 화면이 열리고, 한 판이 시작된다.
2. **메뉴 선택**
   - 오늘의 점심 메뉴 후보(`menu_item`) 2개가 랜덤으로 제시되고, 그 중 하나를 플레이어가 고른다.
