...
## 2. 플레이 구조 요약 (인트로 → 5~7턴, 세력치 100)

- 세력치:
  - 총 인원: **100명**
  - 시작: 플레이어 진영 50 / CPU 진영 50

### 2-1. 인트로 → 배틀 진입 구조 (v1 결정안)

0. **인트로 / Start To Battle 화면**
   - 첫 진입 시에는 바로 배틀 화면이 아니라,  
     - 게임 타이틀 / 한 줄 설명 / “START TO BATTLE” 버튼이 있는 인트로 카드가 나온다.
   - 이 화면에서는:
     - 룰 설명(Claim → Reason → Style 조합, 세력 전쟁 컨셉)을 한 줄로 요약해서 보여주고,
     - 버튼을 누르면 본격적인 배틀 화면으로 전환된다.
1. **한 판 시작 & 컨텍스트 확정**
   - Start 버튼을 누르면 1판이 시작되며,
     - 오늘의 `weather` / `weekday` / `salary_timing`이 정해지고,
     - 상단 정보바(battle-header)에 표시된다.
2. **메뉴 선택**
   - 전체 메뉴 풀에서 2개를 랜덤으로 보여주고, 플레이어가 1개를 고른다.
   - 선택된 메뉴에 따라 `menu_category`와 `faction`이 결정된다.
3. **플레이어 선택 (A안 채택)**
   - **매 턴마다 Claim / Reason / Style 세 가지를 모두 새로 고른다.**
   - Claim → Reason → Style 순서로 UI가 단계적으로 열리며,
     - 한 턴 = “새 문장 빌드업 → 이번 턴 한 방 설득” 구조.
4. **문장 확정**
...
5. **CPU 리액션**
   ...
   - 대신, 플레이어 문장의 **논리적 완성도(데이터/규칙 기반 점수)**에 따라
     - 간단한 리액션(예: “상대가 살짝 수긍”, “고개를 갸웃”, “조금 찔림” 등)을 표현하는 정도로 제한한다.
